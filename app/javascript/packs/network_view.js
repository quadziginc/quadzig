...
axios.get("/views/network/aws_nodes",{
  params: {
  ...
  document.getElementById('showPeeringConnections').addEventListener('change', function(event) {
    if (event.target.checked) {
      axios.get("/views/network/aws_edges").then((resp) => {
        var edges = resp.data
  ...
  cy.on('select', 'node', function(event) {
    ...
    if  ((nodeType !== 'region') &&
      ...
      if (nodeType == "ec2instance") {
        params["vpc_id"] = event.target.data().vpcId
        params["subnet_id"] = event.target.data().subnetId
        params["ec2instance_id"] = event.target.data().id
      }
      ...
      axios.get(`/views/network/${nodeType}_info`, {
        params: params
      }).then((resp) => {
        let infoPanelContainer = infoPanelContainerFactory()
        document.getElementById('network_view-index').append(infoPanelContainer)
  ...
  cy.on('select', 'edge', function(event) {
    ...
    var id_field = 'dbId'
    var params = {}
    params[id_field] = event.target.data().id
    params["account_id"] = event.target.data().accountId
    axios.get(`/views/network/${edgeType}_info`, {
      params: params
    }).then((resp) => {
      let infoPanelContainer = infoPanelContainerFactory()
      document.getElementById('network_view-index').append(infoPanelContainer)
  ...
  document.getElementById('syncResources').onclick = function() {
    mixPanelTrack("infra_sync_triggered", {"source": "network_view_page"})
  ...
  document.getElementById('exportCSVButton').onclick = function(event) {
    ...
    axios.post("/views/network/export_csv", {
      authenticity_token: csrfToken,
      data: {
        resourceIds
      }
...
window.onload = function() {
  mixPanelTrack("visited_network_view", {})
}
